...
if __name__ == "__main__":
    ...
    to_save_dir = os.path.join(base_dir, "..", "data", "vocab.pt")
    os.makedirs(os.path.dirname(to_save_dir), exist_ok=True)


    train_iter = iter_wiki_sentences("train")

    print("counting unigram...")
    unigram, top_pairs, o_id2word, o_word2id = first_pass(train_iter, top_k=5000, min_pair_count=5)
    print("converting new vocab...")
     
    old2new, word2id, id2word, counts = build_vocab(unigram, top_pairs, o_id2word, o_word2id, min_count=20)
    print("merge with top pair")
    counts = torch.from_numpy(counts)
    old2new = torch.from_numpy(old2new)
    ...
    for pair_id in top_pairs:
        ...
        if new_id1 == -1 or new_id2 == -1:
            continue
        if (pos_arr[new_id1], pos_arr[new_id2]) in POS_PAIR or id2word[new_id1] in negate or  (pos_arr[new_id1] == "VERB" and pos_arr[new_id2] == "PART" and pos_arr[new_id2] not in BAD_PART):
    ...
    keep_probs = compute_keep_probs(counts, t=6e-6, adj_mask=adj_ten, adv_mask=adv_ten, common_mask=common_ten)
